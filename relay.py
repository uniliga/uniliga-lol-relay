...
import queue
from util import team_to_url, bans_to_url, team_add_names
...
class EventReplay:
    def __init__(self, data_queue: queue.Queue):
        ...
        self.lcu_connection = None  # Will be set when available
        
    def set_lcu_connection(self, connection):
        """Set the LCU connection for name lookups during replay"""
        self.lcu_connection = connection
        print("LCU connection set for replay system")
    
    def clear_lcu_connection(self):
        """Clear the LCU connection"""
        self.lcu_connection = None
        print("LCU connection cleared from replay system")
                    ...
                    if line:  # Skip empty lines
                        try:
                            ...
                            self.events.append({
                                ...
                                'original_timestamp': event_data.get('timer', {}).get('internalNowInEpochMs', 0)
                            })
    ...
    async def process_event_data(self, event_data: Dict) -> Dict:
        ...
        if data.get('myTeam'):
            ...
            # Try to add names if LCU connection is available
            if self.lcu_connection:
                try:
                    myTeam = await team_add_names(self.lcu_connection, myTeam)
                    print("Added live names to myTeam during replay")
                except Exception as e:
                    print(f"Could not fetch names for myTeam during replay: {e}")
            data['myTeam'] = myTeam
        ...
        if data.get('theirTeam'):
            ...
            if self.lcu_connection:
                try:
                    theirTeam = await team_add_names(self.lcu_connection, theirTeam)
                    print("Added live names to theirTeam during replay")
                except Exception as e:
                    print(f"Could not fetch names for theirTeam during replay: {e}")
        ...
        data['_replay'] = {
            ...
            'original_timestamp': event_data.get('timer', {}).get('internalNowInEpochMs', 0),
            'has_live_connection': self.lcu_connection is not None
        }
    ...
    async def play_events(self, start_index: int = 0, end_index: Optional[int] = None) -> None:
        ...
        print(f"LCU connection available: {self.lcu_connection is not None}")
        ...
        try:
            for i in range(start_index, end_index):
                ...
                # Process the event data (now async to handle name lookups)
                processed_data = await self.process_event_data(event['data'])
    ...
    async def play_events(self, start_index: int = 0, end_index: Optional[int] = None) -> None:
        ...
        try:
            for i in range(start_index, end_index):
                ...
                processed_data = self.process_event_data(event['data'])
